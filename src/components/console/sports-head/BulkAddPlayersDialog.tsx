...
import { sportsHeadBulkAddPlayers, type FullSportsHeadTeam } from '@/lib/api';
...
export function BulkAddPlayersDialog({ team, isOpen, onClose, onSuccess }: BulkAddPlayersDialogProps) {
    ...
    const onSubmit = async (data: FormValues) => {
        try {
            const result = await sportsHeadBulkAddPlayers(team.id, data.players);
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-4xl">
                <DialogHeader>
                    ...
                    <DialogDescription>Manually enter new player details. Only Name is required.</DialogDescription>
                </DialogHeader>
                <Form {...form}>
