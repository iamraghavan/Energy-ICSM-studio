  ...
  return (
    ...
    >
      ...
      <div className="p-6 sm:p-8">
        <div className="flex flex-col sm:grid sm:grid-cols-[1fr,auto,1fr] items-center gap-6 sm:gap-10">
          
          {/* Team A */}
          <div className="flex items-center sm:flex-col sm:items-start justify-between w-full sm:w-auto space-y-1">
            <h3 className="font-black text-lg sm:text-xl text-slate-900 dark:text-slate-100 leading-none uppercase tracking-tight sm:line-clamp-2">
              {teamA.name}
            </h3>
            {/* Mobile Score A */}
            <span className="sm:hidden text-4xl font-black tabular-nums text-slate-950 dark:text-white tracking-tighter">
              {teamA.score}
            </span>
          </div>

          {/* VS & Desktop Scores */}
          <div className="hidden sm:flex items-center justify-center gap-6 sm:gap-10 shrink-0">
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="h-8 w-px bg-slate-200 dark:bg-slate-800 mb-1" />
              ...
              <div className="h-8 w-px bg-slate-200 dark:bg-slate-800 mt-1" />
            </div>
          ...
          {/* Team B */}
          <div className="flex items-center flex-row-reverse sm:flex-col sm:items-end justify-between w-full sm:w-auto space-y-1">
            <h3 className="font-black text-lg sm:text-xl text-slate-900 dark:text-slate-100 leading-none text-right uppercase tracking-tight sm:line-clamp-2">
              {teamB.name}
            </h3>
            {/* Mobile Score B */}
            <span className="sm:hidden text-4xl font-black tabular-nums text-slate-950 dark:text-white tracking-tighter">
          ...
          {/* Mobile "VS" Label */}
          <div className="sm:hidden flex items-center gap-4 w-full opacity-20">
            <div className="h-px flex-1 bg-slate-900 dark:bg-white" />
            <span className="text-[10px] font-black tracking-[0.3em] italic">VS</span>
            <div className="h-px flex-1 bg-slate-900 dark:bg-white" />
          </div>
        </div>
        ...
        {/* Bottom Info Row */}
        {metaInfo && (
          <div className="mt-8 pt-5 border-t border-slate-100 dark:border-slate-800/50 flex justify-center">
            <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] bg-slate-50 dark:bg-slate-800/50 px-5 py-1.5 rounded-full border border-slate-100 dark:border-slate-700/50 shadow-inner">
              {metaInfo}
            </p>
          </div>
        )}
      ...
      <div className={cn(
        ...
        isLive ? "bg-red-600 shadow-[0_-4px_10px_rgba(220,38,38,0.3)]" : "bg-slate-100 dark:bg-slate-800"
      )} />
    </motion.div>
  );
}
